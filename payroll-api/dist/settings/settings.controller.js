...
let SettingsController = class SettingsController {
    ...
    getLogo(res) {
        ...
        for (const ext of exts) {
            const p = (0, path_1.join)(process.cwd(), 'uploads', 'logo' + ext);
            if (fs.existsSync(p))
    ...
    getSignature(res) {
        const exts = ['.png', '.jpg', '.jpeg'];
        for (const ext of exts) {
            const p = (0, path_1.join)(process.cwd(), 'uploads', 'signature' + ext);
            if (fs.existsSync(p))
                return res.sendFile(p);
        }
        return res.status(404).send('Not found');
...
__decorate([
    ...
    (0, common_1.UseInterceptors)((0, platform_express_1.FileInterceptor)('file', {
        storage: (0, multer_1.diskStorage)({
            destination: './uploads',
            filename: (req, file, cb) => cb(null, 'logo' + (0, path_1.extname)(file.originalname))
        })
    })),
...
__decorate([
    ...
    (0, common_1.UseInterceptors)((0, platform_express_1.FileInterceptor)('file', {
        storage: (0, multer_1.diskStorage)({
            destination: './uploads',
            filename: (req, file, cb) => cb(null, 'signature' + (0, path_1.extname)(file.originalname))
        })
    })),
    __param(0, (0, common_1.UploadedFile)()),
    __metadata("design:type", Function),
