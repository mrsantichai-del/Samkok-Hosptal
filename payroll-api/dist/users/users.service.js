...
let UsersService = class UsersService {
    ...
    async getRole(id) {
        ...
        if (!role)
            throw new common_1.NotFoundException('ไม่พบกลุ่มผู้ใช้งานนี้');
        return {
            ...role,
    ...
    async createRole(data) {
        if (!data.name || !data.name.trim()) {
            throw new common_1.BadRequestException('กรุณาระบุชื่อกลุ่มผู้ใช้งาน');
        }
        ...
        if (existing) {
            throw new common_1.BadRequestException(`ชื่อกลุ่มผู้ใช้งาน "${trimmedName}" มีอยู่ในระบบแล้ว`);
        }
    ...
    async removeRole(id) {
        ...
        if (role.users.length > 0) {
            throw new common_1.BadRequestException(`ไม่สามารถลบกลุ่ม "${role.name}" ได้เนื่องจากมีผู้ใช้งานอยู่ในกลุ่มนี้ ${role.users.length} บัญชี กรุณาย้ายผู้ใช้งานไปกลุ่มอื่นก่อนลบ`);
        }
        if (['System Administrator', 'Admin', 'HR', 'Executive', 'Employee'].includes(role.name)) {
            throw new common_1.BadRequestException(`กลุ่ม "${role.name}" เป็นกลุ่มหลักของระบบ ไม่สามารถลบได้`);
        }
