...
let PayrollService = class PayrollService {
    ...
    async processPayroll(dto, userId) {
        ...
        if (existing) {
            if (existing.status === 'APPROVED' || existing.status === 'PAID')
                throw new common_1.BadRequestException('รอบเงินเดือนนี้ได้รับการอนุมัติแล้ว ไม่สามารถประมวลผลซ้ำได้');
        ...
        for (const emp of eligibleEmployees) {
            for (const item of payItems) {
                let amount = 0;
                if (item.name === 'เงินเดือน')
                    amount = Number(emp.baseSalary) || 15000;
    ...
    async getHeadcountSummary(recordId) {
    ...
    async getPayrollRecords() {
        try {
            ...
            const rawRecords = await this.prisma.client.$queryRawUnsafe(`
    ...
    async updateEmployeeTransactions(recordId, employeeId, transactions, userId) {
        ...
        if (!record || record.status !== 'DRAFT')
            throw new common_1.BadRequestException('Cannot edit transactions for approved/paid payroll');
    ...
    async requestEdit(recordId, reason, userId) {
        const record = await this.prisma.payrollRecord.findUnique({ where: { id: recordId } });
        if (!record)
            throw new common_1.NotFoundException('Record not found');
        if (record.status !== 'APPROVED' && record.status !== 'PENDING_APPROVAL') {
            throw new common_1.BadRequestException('สามารถขอแก้ไขได้เฉพาะรอบที่อนุมัติแล้วหรือรออนุมัติเท่านั้น');
        }
    ...
    async grantEdit(recordId, userId) {
        ...
        if (record.status !== 'EDIT_REQUESTED') {
            throw new common_1.BadRequestException('รอบเงินเดือนนี้ไม่ได้อยู่ในสถานะร้องขอแก้ไข');
        }
        await this.prisma.payrollRecord.update({
            where: { id: recordId },
            data: {
    ...
    async rejectEdit(recordId, userId, rejectReason) {
        const record = await this.prisma.payrollRecord.findUnique({ where: { id: recordId } });
        if (!record)
            throw new common_1.NotFoundException('Record not found');
        if (record.status !== 'EDIT_REQUESTED') {
    ...
    async deletePayrollRecord(recordId, userId) {
        const record = await this.prisma.payrollRecord.findUnique({ where: { id: recordId } });
        if (!record)
            throw new common_1.NotFoundException('ไม่พบข้อมูลรอบเงินเดือนที่ต้องการลบ');
        if (record.status === 'APPROVED' || record.status === 'PAID') {
            throw new common_1.BadRequestException('ไม่สามารถลบรอบเงินเดือนที่อนุมัติแล้วได้');
        }
        await this.prisma.payrollTransaction.deleteMany({
            where: { payrollRecordId: recordId }
        });
